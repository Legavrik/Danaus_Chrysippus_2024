...
nb_snp=0
Lchr=0
for chro in range(1, 30):  # Adjusted range to cover chromosomes chr2 to chr31
    print('Processing chromosome',chro)

    # Construct the input file path using the formatted chromosome name
    infile_vcf = 'chromlinks/chr'+str(chro)+'.vcf'
...
# print the result
np.savetxt('res/test_'+pop+'_n'+str(n)+'_mac'+str(mac)+'_macld'+str(mac_ld)+'.stat',np.concatenate((np.array([np.float(nb_snp)/np.float(Lchr)],dtype='float'),res_afs[0],res_ld_zyg[0],res_ibs)),fmt='%.3e')
