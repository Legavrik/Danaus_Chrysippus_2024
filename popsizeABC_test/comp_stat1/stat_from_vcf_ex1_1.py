...
pop='Afr'
list_ani=IO.read_list('data/indiv_Afr') # list of diploid animals used for computing the summary statistics
n=len(list_ani)*2 # haploid sample size
mac=3 # minor allele count threshold for AFS and IBS statistics computation
mac_ld=3 # minor allele count threshold for LD statistics computation
...
r=4.23*10**(-8) # the per generation per bp recombination rate (an approximation is sufficient value)
...
nb_snp=0
Lchr=0
for chro in range(2, 3):  # Adjusted range to cover chromosomes chr2 to chr31
    # Check if current chro is in the list of chromosomes to skip
...
# print the result
np.savetxt('res/chr02_'+pop+'_n'+str(n)+'_mac'+str(mac)+'_macld'+str(mac_ld)+'.stat',np.concatenate((np.array([np.float(nb_snp)/np.float(Lchr)],dtype='float'),res_afs[0],res_ld_zyg[0],res_ibs)),fmt='%.3e')
